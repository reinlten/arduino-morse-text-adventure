...
void getMorseCode(){
  ...
  do{
    ...
    //Serial.println(button_times_high[0]);
  ...
  */
  ...
  for(int i = 0; i< high_counter; i++){
    if(button_times_high[i] > avg_high_time){
      code += "-";
    }else{
      code += ".";
    }
    if(button_times_low[i] > avg_low_time){
      code += " ";
    }
  }
  ...
  code_text = morseToText(code);
  
  Serial.println(code_text);
  
  for(int i = 0; i < code_text.length(); i++){
    buffer[i] = code_text[i];
  }
...
String morseToText(String morseCode) {
  String decodedText = "";  // Hier wird der dekodierte Text gespeichert
  String currentCode = "";  // Hier wird das aktuelle Morsecode-Zeichen gespeichert

  // Schleife durch jeden Charakter im Morsecode-String
  for (int i = 0; i < morseCode.length(); i++) {
    char c = morseCode.charAt(i);

    if (c != ' ') {  // Wenn der Charakter kein Leerzeichen ist, füge ihn zum aktuellen Code hinzu
      currentCode += c;
    } else {  // Wenn ein Leerzeichen gefunden wird, dekodiere das aktuelle Zeichen
      decodedText += decodeMorse(currentCode);
      currentCode = "";  // Setze den aktuellen Code zurück für das nächste Zeichen
    }
  }

  // Das letzte Morsecode-Zeichen dekodieren, falls der String nicht mit einem Leerzeichen endet
...
// Funktion zum Dekodieren eines einzelnen Morsecode-Zeichens
...
void loop() {
  ...
  while (!gameOver) {
    ...
    if (code_text.startsWith("HELP")) {
      doHelp();
    } else if (code_text.startsWith("INVENTORY")) {
      doInventory();
    } else if (code_text.startsWith("GO")){
      code_text = "GO " + code_text.substring(2);
      code_text.toCharArray(buffer,40);
      doGo();
    } else if (code_text.startsWith("LOOK")) {
      doLook();
    } else if (code_text.startsWith("TAKE")) {
      code_text = "TAKE " + code_text.substring(4);
      code_text.toCharArray(buffer,40);
      doTake();
    } else if (code_text.startsWith("EXAMINE")) {
      code_text = "EXAMINE " + code_text.substring(7);
      code_text.toCharArray(buffer,40);
      doExamine();
    } else if (code_text.startsWith("USE")) {
      code_text = "USE " + code_text.substring(3);
      code_text.toCharArray(buffer,40);
      doUse();
    } else if (code_text.startsWith("DROP")) {
